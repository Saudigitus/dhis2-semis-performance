...
import { useGetAttributes, useGetSectionTypeLabel, useSearchEnrollments, useUrlParams } from "dhis2-semis-functions"
import { ProgramConfig, RenderHeader, RenderRows, Table, useDataStoreKey, useProgramsKeys, WithBorder, WithPadding } from "dhis2-semis-components";
import { ModalSearchTemplateProps } from "src/types/modal/ModalProps";
...
function ModalSearchEnrollmentContent(props: ModalSearchTemplateProps): React.ReactElement {
  ...
  const { setOpen, setOpenNewEnrollment } = props;
  const { sectionName } = useGetSectionTypeLabel();
  const { registration, defaults, program } = useDataStoreKey({ sectionType: sectionName })
  const [showResults, setShowResults] = useState<boolean>(false)
  const { programsValues } = useProgramsKeys()


  const selectedProgram = programsValues.find((p) => p.id === program)

  const { attributes: teiAttributes, searchable: searchableAttributes } = useGetAttributes(selectedProgram);
  ...
  const [initialValues] = useState<object>({
    ...
    [registration?.academicYear || defaults?.currentAcademicYear ?? ""]: academicYear
  })
